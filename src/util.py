...
def random_draw(rankerA_prec, rankerB_prec, overlap, index):
    ...
    left_index = indexList[rankerB_end:]
    rankerA_index.sort()
    rankerB_index.sort()
    left_index.sort()
    return rankerA_index, rankerB_index, left_index
...
def save_doc(path, rankerA_query, rankerB_query, left_query, queryname):
    if not os.path.exists(path):
        os.makedirs(path)
    queryList = [rankerA_query, rankerB_query, left_query]
...
def main():
    ...
    parser.add_argument('-n', '--query_name', help='query names', nargs='+', type=str)
    ...
    rankerA_index, rankerB_index, left_index = random_draw(FLAGS.ranker_prop[0], FLAGS.ranker_prop[1], FLAGS.ranker_prop[2],
                                               len(queries))

    rankerA_query = [queries[i] for i in rankerA_index]
    rankerB_query = [queries[i] for i in rankerB_index]
    left_query = [queries[i] for i in left_index]
    save_doc(FLAGS.output_dir, rankerA_query, rankerB_query, left_query, FLAGS.query_name)
